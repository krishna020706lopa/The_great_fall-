...
var music;
var you,You; 
var guilty,guiltyImage;
var dreams,dreamsImage;
var fall,fallImage;
var lime;
var logi;
var warImage1,warImage2,warImage3,warImage4,war1,war2,war3,war4;
...
function preload() {
  ...
  music=createAudio("gravity.mp3");
  music1=createAudio("Come-Play-with-Me.mp3");
  music2=createAudio("creepy_sound.mp3");
  You=loadImage("lastword/villan.png");
  dreamsImage=loadImage("lastword/warpPad.png");
  lime=loadImage("lastword/limbo.png");
  fallImage=loadImage("lastword/limbo real.png");
  guiltyImage=loadImage("lastword/dream.png");
  warImage1=loadImage("lastword/house.png");
  warImage2=loadImage("lastword/family.png");
  warImage3=loadImage("lastword/falling.png");
  warImage4=loadImage("lastword/dream_falling.png");
}
...
function setup(){
  ...
  red1.scale=0.2;
  fall=createSprite(2000,300,300,300);
  fall.addImage("noooo",fallImage);
 // fall.scale=2;
  guilty=createSprite(1910,185,10,10);
  guilty.addImage("the fall",guiltyImage);
  guilty.scale=0.8;
  logi=createSprite(1910,400,10,10);
  ...
  dreams.scale=0.05;
  war1=createSprite(200,2700,300,127);
  war1.addImage("my house",warImage1);
  war2=createSprite(600,2700,300,127);
  war2.addImage("family",warImage2);
  war3=createSprite(1100,2700,300,127);
  war3.addImage("dream falling",warImage3);
  war4=createSprite(1700,2700,300,127);
  war4.addImage("falling",warImage4);
}
...
if(logi.isTouching(guilty)&&story<1&&story>-1){
  guilty.position.x=1910;
  guilty.position.y=185;
  story=1;
  guilty.velocityY=0;
}
if(story<2&&story>0){
...
if(logi.isTouching(guilty)&&story<2&&story>0){
  guilty.position.x=1910;
  guilty.position.y=185;
  story=2;
  guilty.velocityY=0;
...
if(logi.isTouching(guilty)&&story<3&&story>1){
  guilty.position.x=1910;
  guilty.position.y=185;
  story=3;
  guilty.velocityY=0;
...
if(logi.isTouching(guilty)&&story<4&&story>2){
  guilty.position.x=1910;
  guilty.position.y=185;
  story=4;
  guilty.velocityY=0;
...
if(piano.isTouching(dream)&&keyDown("space")&&help<5&&help>3){
  dream.x=2300;
  dream.y=2145;
}
...
if(logi.isTouching(guilty)&&story<5&&story>3){
  guilty.velocityY=0;
  gameState=END;
}
if(sorry.isTouching(dream)&&tears<1&&tears>-1){
  console.log("Qay-siri?");
  console.log("Qay-what are you doing here?");
  console.log("Siri-The clue is where you find me and sorry lies");
  console.log("Qay-what?,why are you acting differently?");
  console.log("Siri-Uoy dluoc evas em ;<");
  tears=1;
}
if(savior.isTouching(dream)&&tears<2&&tears>0){
 console.log("Qay-ryan?");
 console.log("Qay-ryan what are doing here and why siri is acting different?");
 console.log("Ryan-Hello sir the clue is where you I save a killer");
 console.log("Qay-No,I don't kill anyone nooooooo!");
 console.log("Ryan-Uoy era a rellik");
 tears =2;
}
if (helper.isTouching(dream)&&tears<3&&tears>1){
  console.log("Qay-hendry....");
  console.log("Qay-Why,why?are doing this clad why.......;<");
  console.log("Hendry-The next clue is where I help a selfish");
  console.log("Hendry-Uoy era hsifles");
  tears =3;
}
if(friend.isTouching(dream)&&tears<4&&tears>2){
  console.log("Qay-noooooooo not lam");
  console.log("Lam-the next clue is where I found an enemy");
  console.log("Lam-Uoy era ton ym dneirf");
  tears =4;
}
if(car.isTouching(dream)&&tears<5&&tears>3){
  console.log("Qay-cam....");
  console.log("Ouy dluoc evas em")
  tears =5;
}
if(Imposter.isTouching(dream)&&tears<6&&tears>4){
  console.log("Qay-imortal....");
  console.log("Ouy dluoc evas em");
  tears =6;
}
if(angry.isTouching(dream)&&tears<7&&tears>5){
  console.log("Qay-amy....");
  console.log("Ouy dluoc evas em");
  tears =7;
}
if(brother.isTouching(dream)&&tears<8&&tears>6){
  console.log("Qay-imortal....");
  console.log("Ouy nac evas em");
  tears =8;
}
dream.collide(wall1);
dream.collide(wall2);
...
dream.collide(you);
logi.collide(guilty);
dream.collide(savior);
dream.collide(sorry);
dream.collide(helper);
dream.collide(friend);
dream.collide(car);
dream.collide(Imposter);
dream.collide(trex);
dream.collide(brother);
piano.visible=0;
note1.visible=0;
note2.visible=0;
note3.visible=0;
